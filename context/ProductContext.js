...
export const ProductProvider = ({ children }) => {
    ...
    const newProduct = async (product) => {
        try {
            const { data } = await axios.post(`${process.env.API_URL}/api/admin/products`,
                product
            );
    ...
    const postReview = async (reviewData) => {
        try {
            const { data } = await axios.put(`${process.env.API_URL}/api/products/review`,
                reviewData
            );
    ...
    const uploadProductImages = async (formData, id) => {
        try {
            ...
            const { data } = await axios.post(`${process.env.API_URL}/api/admin/products/upload_images/${id}`,
                formData,
                {
                    headers: {
